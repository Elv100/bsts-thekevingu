...
/*
 * Kevin's java
 * A functioning balancing binary search tree with drawing and printing methods
 */
public class BalancedBinarySearchTree<T> {
    ...
    private Node put(Node node, int key, T element) {
        ...
        /* shift once */
        if (difference > 1 && key < node.left.key)
            return right(node);
        if (difference < -1 && key > node.right.key)
            return left(node);
    ...
    private Node delete(Node node, int key) {
        ...
        /* depending on differences of subtrees, shift once */
        if (difference > 1 && heightDifference(node.left) >= 0)
            return right(node);
        if (difference < -1 && heightDifference(node.right) <= 0)
            return left(node);

        /* depending on differences of subtrees, shift twice */
        if (difference < -1 && heightDifference(node.right) > 0) {
            node.right = right(node.right);
            return left(node);
        }
    ...
    private int heightDifference(Node N) {
        if (N == null)
            return 0;
